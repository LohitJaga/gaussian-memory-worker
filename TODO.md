...
- [x] Entity graph retrieval — entity_nodes + memory_entities tables created. 1-hop traversal in retrieve(): query tokens → lookup entity_nodes → pull connected memory_ids → boost +0.1/shared entity (max 0.2). Retroactive batch job (memory_build_entities): 6,369 memories processed via Llama. Forward extraction: new memories queued to KV pending_entity_queue on store, processPendingEntityQueue() runs in cron nightly (50/run). 1,528 unique entities, 3,606 links. memory_process_entity_queue test tool added. (June 3)
