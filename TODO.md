...
- [ ] Entity graph retrieval — 2 new tables: entity_nodes (type, canonical_name, embedding, last_seen) + memory_entities (memory_id, entity_id, entity_span, confidence). 1-hop traversal at retrieve(): extract entities from query → lookup memory_entities → boost +0.15/shared entity. Backfill via batch job on top-accessed memories. Forward extraction on every store(). Entity types: tool (@cf/, GLM-4.7-flash), project (Gaussian Memory, Color Wow), concept (spreading activation), parameter (topK=2), person. Stays in SQLite, no graph DB needed.
